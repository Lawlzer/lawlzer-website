...
// Function to get the appropriate URL - client-side only
function getNavUrl(subdomain?: string | null): string {
...
const NavLink = ({ href, children }: { href: string; children: React.ReactNode }) => {
	...
	return (
		<ProtectedLink href={href} className={`relative px-4 py-2 text-sm font-medium transition-all duration-200 rounded-lg ${isActive ? 'text-primary bg-primary/10' : 'text-secondary-text hover:text-foreground hover:bg-secondary/50'} group`}>
			{children}
...
function DesktopNav({ getUrl }: { getUrl: (key: string) => string }): React.JSX.Element {
	return (
...
function TopbarRightSection({ session, isOpen, setIsOpen }: { session: SessionData | null; isOpen: boolean; setIsOpen: (v: boolean) => void }): React.JSX.Element {
	return (
...
function TopbarMobileMenu({ isOpen, getUrl, setIsOpen }: { isOpen: boolean; getUrl: (key: string) => string; setIsOpen: (v: boolean) => void }): React.JSX.Element | null {
		...
		<>
			...
			>
				<div className='mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8'>
					<DesktopNav getUrl={getUrl} />
					<TopbarRightSection session={session} isOpen={isOpen} setIsOpen={setIsOpen} />
				</div>
				<TopbarMobileMenu isOpen={isOpen} getUrl={getUrl} setIsOpen={setIsOpen} />
			</nav>
			<AnimatePresence>
				{isOpen && (
					<motion.div
