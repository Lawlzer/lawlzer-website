...
const AuthButton = ({ initialSession }: AuthButtonProps): React.JSX.Element => {
	...
	if (loading) {
		return <div className='h-9 w-20 animate-pulse rounded-lg bg-secondary' data-testid='auth-loading' />;
	}
	...
	if (user) {
		return (
			<Menu as='div' className='relative inline-block text-left'>
				<div>
					<MenuButton className='inline-flex items-center gap-2 rounded-lg bg-secondary px-4 py-2 text-sm font-medium text-secondary-foreground transition-all hover:bg-secondary/80 focus:outline-none focus:ring-2 focus:ring-primary/50'>
						{user.name ?? user.email ?? 'Account'}
						<ChevronDownIcon aria-hidden='true' className='h-4 w-4 text-secondary-text' />
					</MenuButton>
				</div>
				<Menu.Items className='absolute right-0 z-50 mt-2 w-56 origin-top-right divide-y divide-border rounded-lg bg-popover shadow-lg ring-1 ring-border focus:outline-none' data-testid='menuitems'>
					<div className='p-1'>
						<Menu.Item>
							{({ active }) => (
								<a
									className={cn('group flex w-full items-center rounded-md px-3 py-2 text-sm transition-colors', active ? 'bg-accent text-accent-foreground' : 'text-foreground')}
	...
	return (
		<Menu as='div' className='relative inline-block text-left'>
			<div>
				<MenuButton className='inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground shadow-sm transition-all hover:bg-primary/90 hover:shadow focus:outline-none focus:ring-2 focus:ring-primary/50'>
					Login / Register
				...
				<div className='p-1'>
					{[
						...
						<Menu.Item key={provider}>
							{({ active }) => (
								<a
									className={cn('group flex w-full items-center rounded-md px-3 py-2 text-sm transition-colors', active ? 'bg-accent text-accent-foreground' : 'text-foreground', disabled && 'cursor-not-allowed opacity-50')}
