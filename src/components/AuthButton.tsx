...
const AuthButton = ({ initialSession }: AuthButtonProps): React.JSX.Element => {
	...
	if (user) {
		return (
			<Menu as='div' className='relative inline-block text-left'>
				<div>
					<MenuButton className='bg-secondary text-secondary-foreground hover:bg-secondary/80 focus-visible:ring-ring focus-visible:ring-offset-background inline-flex w-full justify-center rounded-md px-4 py-2 text-sm font-medium focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2'>
						{user.name ?? user.email ?? 'Account'}
						<ChevronDownIcon aria-hidden='true' className='text-muted-foreground -mr-1 ml-2 h-5 w-5' />
					</MenuButton>
				</div>
				<Menu.Items className='divide-border bg-popover text-popover-foreground ring-border absolute right-0 z-50 mt-2 w-56 origin-top-right divide-y rounded-md shadow-lg ring-1 focus:outline-none' data-testid='menuitems'>
					<div className='px-1 py-1'>
						<Menu.Item>
							{({ active }) => (
								<a
									className={cn('group flex w-full items-center rounded-md px-2 py-2 text-sm', active ? 'bg-accent text-accent-foreground' : 'text-foreground')}
	...
	return (
		<Menu as='div' className='relative inline-block text-left'>
			<div>
				<MenuButton className='bg-primary text-primary-foreground hover:bg-primary/90 focus-visible:ring-ring focus-visible:ring-offset-background inline-flex w-full justify-center rounded-md px-4 py-2 text-sm font-medium focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2'>
					Login / Register
					<ChevronDownIcon aria-hidden='true' className='text-primary-foreground/80 -mr-1 ml-2 h-5 w-5' />
				</MenuButton>
			</div>
			<Menu.Items className='divide-border bg-popover text-popover-foreground ring-border absolute right-0 z-50 mt-2 w-56 origin-top-right divide-y rounded-md shadow-lg ring-1 focus:outline-none' data-testid='menuitems'>
				<div className='px-1 py-1'>
					{[
						...
						},
						{
							...
							label: 'GitHub',
							icon: (
								<svg className='mr-2 h-5 w-5' viewBox='0 0 127.14 96.36' xmlns='http://www.w3.org/2000/svg'>
					...
					].map(({ provider, label, icon, disabled }) => (
						<Menu.Item key={provider}>
							{({ active }) => (
								<a
									className={cn('group flex w-full items-center rounded-md px-2 py-2 text-sm', active ? 'bg-accent text-accent-foreground' : 'text-foreground', disabled && 'cursor-not-allowed opacity-50')}
