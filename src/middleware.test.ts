...
describe('middleware', () => {
  ...
  describe('Development Redirects', () => {
    it('should rewrite localhost to root subdomain in development', () => {
      ...
      expect(response).toEqual({
        type: 'rewrite',
        url: 'http://localhost:3000/subdomains/root/test-path',
      });
    });
  ...
  describe('Subdomain Routing', () => {
    ...
    it('should pass through _next paths on subdomains', () => {
      const request = createMockRequest(
        'http://colors.dev.localhost:3000/_next/static/chunk.js',
        {
          host: 'colors.dev.localhost:3000',
        }
      );
    ...
    it('should rewrite non-root paths on subdomains', () => {
      const request = createMockRequest(
        'http://colors.dev.localhost:3000/about',
        {
          host: 'colors.dev.localhost:3000',
        }
      );
  ...
  describe('Header Handling', () => {
    it('should use host header when both host and x-forwarded-host are present', () => {
      // The middleware currently prioritizes host over x-forwarded-host
      ...
      // Should use host header (dev.localhost) not x-forwarded-host
      expect(response).toEqual({
        type: 'rewrite',
    ...
    it('should use x-forwarded-host when host is not present', () => {
      // Only use x-forwarded-host when host is missing
      const request = createMockRequest('http://dev.localhost:3000/', {
        'x-forwarded-host': 'www.dev.localhost:3000',
      });

      const response = middleware(request);

      // Should redirect to remove www prefix
      expect(response).toEqual({
        type: 'redirect',
  ...
  describe('Debug Mode', () => {
    it('should log debug messages when DEBUG_SUBDOMAIN_VALUE is true', () => {
      ...
      expect(consoleMock.spy).toHaveBeenCalledWith(
        '[Middleware] Running middleware for:',
        'http://colors.dev.localhost:3000/'
      );
      expect(consoleMock.spy).toHaveBeenCalledWith(
        '[Middleware] Detected hostname: colors.dev.localhost'
      );
