...
export function useRecipeForm(initialState: Partial<RecipeFormState> = {}) {
	...
	const addIngredient = useCallback(
		(availableFoods: Food[], availableRecipes: RecipeWithDetails[]) => {
			...
			if (selectedType === 'food') {
				...
				const nestedRecipe = availableRecipes.find((r) => r.id === selectedRecipeId);
				if (!nestedRecipe) {
	...
	const updateIngredient = useCallback((index: number, updates: Partial<RecipeItem>) => {
		setFormData((prev) => {
	...
	const calculateTotalNutrition = useCallback(() => {
		...
		formData.items.forEach((item) => {
			...
			if (item.food) {
				...
				totalCalories += currentVersion.caloriesPerServing * recipeServings * servingFactor;
				totalProtein += currentVersion.proteinPerServing * recipeServings * servingFactor;
				totalCarbs += currentVersion.carbsPerServing * recipeServings * servingFactor;
				totalFat += currentVersion.fatPerServing * recipeServings * servingFactor;
			}
		});
