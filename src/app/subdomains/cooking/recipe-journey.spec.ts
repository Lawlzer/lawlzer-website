...
test.describe('Cooking Subdomain - Recipe Journey', () => {
  ...
  test('should display the cooking homepage with all tabs', async ({
    page,
  }) => {
    // Check main heading
    await expect(
      page.getByRole('heading', { name: 'Cooking & Nutrition' })
    ).toBeVisible();
  ...
  test('should show empty state when no recipes exist', async ({ page }) => {
    ...
    await expect(
      page.getByText('Create your first recipe to start building your cookbook')
    ).toBeVisible();
    await expect(
      page.getByRole('button', { name: 'Create Your First Recipe' })
    ).toBeVisible();
  });
  ...
  test('should search for recipes', async ({ page }) => {
    ...
    await expect(
      page.getByText(/pasta/i).or(page.getByText('No results found'))
    ).toBeVisible();
  });
  ...
  test('should navigate through different tabs', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: /Nutrition Goals/i })
    ).toBeVisible();
  ...
  test('should show recipe details when clicking on a recipe', async ({
    page,
  }) => {
    ...
    if (await recipeCard.isVisible()) {
      ...
      await expect(
        page
          .getByRole('heading', { name: /Edit Recipe/i })
          .or(page.getByText(/Ingredients/i))
      ).toBeVisible();
    }
  });
  ...
  test('should access cooking mode', async ({ page }) => {
    ...
    await expect(
      page.getByText(/Cooking Mode/i).or(page.getByText(/Select a recipe/i))
    ).toBeVisible();
  });
