...
test.describe('Guest Mode Functionality', () => {
	...
	test('should show guest mode banner when not logged in', async ({ page }) => {
		...
		await expect(page.locator('text=/Sign in to save your data permanently/i')).toBeVisible();
	});
	...
	test('should store recipes in local storage for guests', async ({ page }) => {
		...
		await page.fill('textarea[placeholder="Description"]', 'A test recipe created by a guest');
	...
	test('should store goals for guests', async ({ page }) => {
		...
		// Set some goals
		const calorieInput = page.locator('input[placeholder*="calorie" i]').first();
		...
		const calorieValue = await page.locator('input[placeholder*="calorie" i]').first().inputValue();
		if (calorieValue) {
	...
	test('should show migration prompt when signing in', async ({ page }) => {
		...
		await page.click('text=Recipes');
		await page.click('text=Create New Recipe');
		await page.fill('input[placeholder="Recipe name"]', 'Guest Recipe to Migrate');
		await page.fill('textarea[placeholder="Description"]', 'This should be migrated');
		await page.click('button:has-text("Save Recipe")');
	...
	test('should handle guest data limits gracefully', async ({ page }) => {
		...
		for (let i = 0; i < 5; i++) {
			await page.click('text=Create New Recipe');
			await page.fill('input[placeholder="Recipe name"]', `Guest Recipe ${i + 1}`);
			await page.fill('textarea[placeholder="Description"]', `Description ${i + 1}`);
			await page.click('button:has-text("Save Recipe")');
	...
	test('should clear guest data on explicit clear', async ({ page, context }) => {
	...
	test('should maintain separate guest sessions across browsers', async ({ browser }) => {
	...
	test('should handle cooking mode for guests', async ({ page }) => {
		...
		await page.fill('textarea[placeholder="Description"]', 'Test recipe for cooking mode');

		// Add some instructions
		const instructionsInput = page.locator('textarea[placeholder*="instruction" i]').first();
		if (await instructionsInput.isVisible()) {
			await instructionsInput.fill('Step 1: Do this\nStep 2: Do that\nStep 3: Finish');
		}
