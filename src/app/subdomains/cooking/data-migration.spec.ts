...
test.describe('Data Migration Flow', () => {
	test('should show migration dialog when logging in with guest data', async ({ page, context }) => {
		...
		await page.fill('textarea[placeholder="Description"]', 'This recipe should be migrated');
	...
	test('should handle partial migration failures', async ({ page }) => {
		...
		await expect(page.locator('button:has-text("Retry Failed Items")')).toBeVisible();
	});
