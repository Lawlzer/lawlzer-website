...
export async function deleteRecipe(recipeId: string) {
  return fetchApi<void>(`/api/cooking/recipes?id=${recipeId}`, {
    method: 'DELETE',
  });
}

export async function revertRecipeVersion(recipeId: string, versionId: string) {
  return fetchApi<RecipeWithDetails>(
    `/api/cooking/recipes/${recipeId}/revert`,
    {
      method: 'POST',
...
// Day Entries API
export async function createDayEntry(entry: {
  date: string;
  type: 'food' | 'recipe';
  itemId: string;
  quantity: number;
...
export async function deleteDayEntry(entryId: string) {
  return fetchApi<void>(`/api/cooking/days/entries/${entryId}`, {
    method: 'DELETE',
  });
}
