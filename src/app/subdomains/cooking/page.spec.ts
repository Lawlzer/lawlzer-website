...
test.describe('Cooking Page E2E Tests', () => {
  ...
  test('should handle guest mode with local storage', async ({
    page,
    context,
  }) => {
  ...
  test('should search for recipes', async ({ page }) => {
    ...
    await expect(
      page.getByText(/pasta/i).or(page.getByText(/No results found/i))
    ).toBeVisible();
  });
  ...
  test('should use barcode scanner', async ({ page }) => {
    ...
    await expect(page.getByText('Barcode Scanner')).toBeVisible();
    await expect(
      page.getByPlaceholder(/Enter barcode manually/i)
    ).toBeVisible();
    ...
    await expect(
      page.getByText(/Searching/i).or(page.getByText(/Product not found/i))
    ).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should access cooking mode', async ({ page }) => {
    ...
    await expect(
      page
        .getByText(/Cooking Mode/i)
        .or(page.getByText(/Select a recipe to start cooking/i))
    ).toBeVisible();
  ...
  test('should show analysis charts', async ({ page }) => {
    ...
    await expect(
      page
        .getByText(/Weekly Overview/i)
        .or(page.getByText(/No data to display/i))
    ).toBeVisible();
  });
