...
export const KeyboardShortcutsHelp: React.FC = () => {
	...
	useEffect(() => {
		...
		return () => {
			window.removeEventListener('toggle-keyboard-shortcuts-help', handleToggle);
				...
				<div className='bg-white dark:bg-gray-800 rounded-lg shadow-xl'>
						...
						<div className='space-y-6'>
							{shortcutSections.map((section) => (
								<div key={section.title}>
									<h3 className='font-medium text-gray-900 dark:text-gray-100 mb-3'>{section.title}</h3>
												...
												<div className='flex items-center gap-1'>
													{shortcut.keys.map((key, keyIndex) => (
														<React.Fragment key={keyIndex}>
															...
															<kbd className='px-2 py-1 text-xs font-semibold text-gray-800 bg-gray-100 border border-gray-300 rounded dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600'>{key}</kbd>
														</React.Fragment>
													))}
												</div>
											</div>
					...
					<div className='p-4 border-t border-gray-200 dark:border-gray-700'>
						<p className='text-xs text-gray-500 dark:text-gray-400 text-center'>
							Press <kbd className='px-1.5 py-0.5 text-xs font-semibold text-gray-800 bg-gray-100 border border-gray-300 rounded dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600'>?</kbd> to toggle this help
						</p>
					</div>
				</div>
			</div>
