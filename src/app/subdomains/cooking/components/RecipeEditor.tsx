...
export function RecipeEditor({ recipe, availableFoods, availableRecipes = [], onSave, onCancel }: RecipeEditorProps) {
	const [isSubmitting, setIsSubmitting] = useState(false);
	const [showVersionWarning, setShowVersionWarning] = useState(true);
	const [viewingAlternatives, setViewingAlternatives] = useState<string | null>(null);
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			await onSave({
				...
				prepTime: formData.prepTime !== '' && formData.prepTime !== undefined && !Number.isNaN(parseInt(formData.prepTime)) ? parseInt(formData.prepTime) : null,
				cookTime: formData.cookTime !== '' && formData.cookTime !== undefined && !Number.isNaN(parseInt(formData.cookTime)) ? parseInt(formData.cookTime) : null,
				servings: !Number.isNaN(parseInt(formData.servings)) ? parseInt(formData.servings) : 1,
				...
				(item, _index) =>
					viewingAlternatives === item.id && (
						<div key={item.id} className='mt-2'>
							<IngredientAlternatives recipeItemId={item.id} availableFoods={availableFoods} availableRecipes={availableRecipes} />
						</div>
					)
			)}
						...
						return (
							<div key={item.id} className='flex items-center gap-2'>
								...
								<Button
									variant='link'
									onClick={() => {
										setViewingAlternatives(viewingAlternatives === item.id ? null : item.id);
									}}
								>
									{viewingAlternatives === item.id ? 'Hide' : 'View'} Alternatives
								</Button>
							</div>
						);
					})}
		...
		<div className='space-y-4'>
			...
			<form onSubmit={(e) => void handleSubmit(e)} className='space-y-4'>
				...
				<div className='flex justify-end gap-2'>
					<button type='button' onClick={onCancel} className='px-4 py-2 border rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors'>
						Cancel
					</button>
					<button type='submit' disabled={isSubmitting} className='px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors disabled:opacity-50'>
