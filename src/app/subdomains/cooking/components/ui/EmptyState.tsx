...
export function EmptyState({ icon, title, description, action, secondaryAction, className = '' }: EmptyStateProps) {
	...
	return (
		<div className={`text-center py-12 ${className}`}>
			...
			<p className='mt-2 text-gray-600 dark:text-gray-400 max-w-md mx-auto'>{description}</p>
			{(action ?? secondaryAction) && (
				<div className='mt-6 flex gap-2 justify-center'>
					{secondaryAction && (
						<button onClick={secondaryAction.onClick} className='px-4 py-2 text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300'>
                ...
                ${action.variant === 'secondary' ? 'border border-gray-300 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800' : 'bg-blue-500 text-white hover:bg-blue-600'}
              `}
						>
							{action.label}
						</button>
