...
export const ListSkeleton: React.FC<{ count?: number }> = ({ count = 3 }) => (
	<div className='space-y-3'>
		{Array.from({ length: count }).map((_, i) => (
			<div key={i} className={`flex items-center space-x-4 p-3 ${animations.skeleton}`}>
	...
	<div className='w-full'>
		<div className='mb-4 flex space-x-4 border-b pb-4'>
			{Array.from({ length: cols }).map((_, i) => (
				<div key={i} className={`h-4 flex-1 rounded bg-muted ${animations.skeleton}`} />
			))}
		</div>
		{Array.from({ length: rows }).map((_, rowIndex) => (
			<div key={rowIndex} className='flex space-x-4 border-b py-4'>
				{Array.from({ length: cols }).map((__, colIndex) => (
					<div key={colIndex} className={`h-4 flex-1 rounded bg-muted ${animations.skeleton}`} />
				))}
			</div>
		))}
	</div>
...
// Spinner loading
export const Spinner: React.FC<{ size?: 'lg' | 'md' | 'sm' }> = ({ size = 'md' }) => {
	const sizeClasses = {
		sm: 'h-4 w-4',
		md: 'h-8 w-8',
		lg: 'h-12 w-12',
