...
export function RecipeForm({ formData, onInputChange, onAddIngredient, onRemoveIngredient, onUpdateIngredient, availableFoods, availableRecipes, selectedType, onSelectedTypeChange, selectedFoodId, onSelectedFoodIdChange, selectedRecipeId, onSelectedRecipeIdChange, amount, onAmountChange, error, searchTerm = '', onSearchTermChange, nutrition }: RecipeFormProps) {
	// Ensure arrays are never undefined
	const safeFoods = availableFoods ?? [];
	const safeRecipes = availableRecipes ?? [];

	const filteredFoods = searchTerm ? safeFoods.filter((food) => food.name.toLowerCase().includes(searchTerm.toLowerCase())) : safeFoods;

	return (
		<div className='space-y-4'>
			{error !== null && error !== undefined && <div className='p-3 rounded-lg bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400'>{error}</div>}
			...
			<div>
				...
				<p className='text-xs text-gray-500 mt-1'>{formData.visibility === 'private' ? 'Only you can see this recipe.' : formData.visibility === 'unlisted' ? 'Anyone with the link can see this recipe.' : 'This recipe will appear in public search results.'}</p>
			</div>
			...
			<div>
				...
				<p className='text-xs text-gray-500 mt-1'>Component recipes (e.g., sauces, doughs) can be used in other recipes but won&apos;t show up in the main recipe list.</p>
			</div>
			...
			<div>
				...
				{formData.items.length > 0 && (
					<div className='space-y-2 mb-4'>
						{formData.items.map((item, index) => {
							const name = item.food?.name ?? item.recipe?.name ?? 'Unknown Ingredient';
				...
				<div className='space-y-3 p-3 bg-gray-50 dark:bg-gray-900 rounded-lg'>
					...
					<button type='button' onClick={onAddIngredient} className='w-full px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors'>
						Add Ingredient
					</button>
				</div>
			</div>
				...
				<div className='p-4 bg-gray-50 dark:bg-gray-900 rounded-lg'>
					...
					<div className='grid grid-cols-2 gap-4'>
						<div>
							...
							<p className='font-medium'>{nutrition.totalCalories.toFixed(0)} kcal</p>
							<p className='text-sm'>
								P: {nutrition.totalProtein.toFixed(1)}g | C: {nutrition.totalCarbs.toFixed(1)}g | F: {nutrition.totalFat.toFixed(1)}g
							</p>
						</div>
						<div>
							<p className='text-sm text-gray-600 dark:text-gray-400'>Per Serving</p>
							<p className='font-medium'>{nutrition.perServing.calories.toFixed(0)} kcal</p>
							<p className='text-sm'>
								P: {nutrition.perServing.protein.toFixed(1)}g | C: {nutrition.perServing.carbs.toFixed(1)}g | F: {nutrition.perServing.fat.toFixed(1)}g
							</p>
						</div>
					</div>
				</div>
