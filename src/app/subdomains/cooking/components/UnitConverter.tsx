...
export function UnitConverter({ foodId }: UnitConverterProps) {
	...
	useEffect(() => {
		const fetchConversions = async () => {
			setIsLoading(true);
			try {
				const response = await fetch(`/api/cooking/conversions?foodId=${foodId}`);
	...
	const handleVote = async (conversionId: string, voteType: 'downvote' | 'upvote') => {
		try {
	...
	return (
		<div className='mt-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg'>
			...
			{showSuggestForm && (
				...
				>
					<input name='fromUnit' placeholder='e.g., 1 cup of flour' className='w-full p-1 border rounded' />
					<input type='number' name='factor' placeholder='equals how many grams?' className='w-full p-1 border rounded' />
