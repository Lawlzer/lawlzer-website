...
type FilterOption = 'component' | 'private' | 'public';
...
export const RecipeSearch: React.FC<RecipeSearchProps> = ({ recipes, onSearchResults }) => {
	...
	const sortRecipes = useCallback((recipesToSort: RecipeWithDetails[], sort: SortOption): RecipeWithDetails[] => {
	...
	const filteredRecipes = useMemo(() => {
		...
		if (filters.query) {
			...
			results = results.filter((recipe) => {
				...
				if (recipe.currentVersion?.items) {
					return recipe.currentVersion.items.some((item) => item.food?.name.toLowerCase().includes(query) || item.recipe?.name.toLowerCase().includes(query));
				}
		...
		// Servings filter
		if (filters.maxServings !== undefined && filters.maxServings !== null) {
			results = results.filter((recipe) => recipe.servings <= filters.maxServings!);
		}
	...
	const handleVisibilityToggle = (visibility: FilterOption) => {
		setFilters((prev) => ({
			...prev,
			visibility: prev.visibility.includes(visibility) ? prev.visibility.filter((v) => v !== visibility) : [...prev.visibility, visibility],
		}));
	};
	...
	const activeFilterCount = filters.visibility.length + (filters.maxServings !== undefined && filters.maxServings !== null ? 1 : 0);
		...
		<div className='space-y-4'>
			...
			{showFilters && (
				<div className='bg-gray-50 rounded-lg p-4 space-y-4'>
					{/* Visibility Options */}
					<div>
						<h3 className='text-sm font-medium text-gray-700 mb-2'>Recipe Type</h3>
						<div className='flex flex-wrap gap-2'>
							{(['private', 'public', 'component'] as FilterOption[]).map((visibility) => (
								<button
									key={visibility}
									onClick={() => {
										handleVisibilityToggle(visibility);
									}}
									className={`px-3 py-1 rounded-full text-sm transition-colors ${filters.visibility.includes(visibility) ? 'bg-blue-600 text-white' : 'bg-white border border-gray-300 text-gray-700 hover:bg-gray-100'}`}
								>
									{visibility === 'component' ? 'Recipe Component' : visibility.charAt(0).toUpperCase() + visibility.slice(1)}
								</button>
							))}
						</div>
					</div>
					...
					{/* Servings Range */}
					<div>
						<h3 className='text-sm font-medium text-gray-700 mb-2'>Maximum Servings</h3>
						<input
							type='number'
							...
							onChange={(e) => {
								setFilters((prev) => ({
									...prev,
									maxServings: e.target.value ? parseInt(e.target.value) : undefined,
								}));
							}}
							placeholder='e.g. 4'
