...
export const NutritionChart: React.FC<NutritionChartProps> = ({ calories = 0, protein = 0, carbs = 0, fat = 0, showLabels = true, size = 'md' }) => {
	const data = useMemo(() => {
	...
	if (data.length === 0) {
		return (
			<div className={`flex items-center justify-center ${animations.fadeIn}`} style={{ width: config.width, height: config.height }}>
			...
			return (
				<div className='rounded-lg border bg-background p-3 shadow-lg'>
					...
					<p className='text-sm text-muted-foreground'>
						{tooltipData.payload.grams}g ({tooltipData.payload.percentage}%)
					</p>
					<p className='text-sm text-muted-foreground'>{tooltipData.value} calories</p>
				</div>
			);
		}
		return null;
	...
	return (
		<div className={`${animations.fadeIn}`}>
			<ResponsiveContainer width={config.width} height={config.height}>
				<PieChart>
					<Pie data={data} cx='50%' cy='50%' innerRadius={config.innerRadius} outerRadius={config.outerRadius} paddingAngle={2} dataKey='value' animationBegin={0} animationDuration={800}>
