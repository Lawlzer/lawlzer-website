...
export function FullDayNutrition({ recipe, onClose }: FullDayNutritionProps) {
	...
	const { caloriesPerServing, proteinPerServing, carbsPerServing, fatPerServing, fiberPerServing, sugarPerServing, sodiumPerServing } = recipe.currentVersion;
		...
		<div className='fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4'>
			<div className='bg-white dark:bg-gray-900 rounded-2xl p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-hidden flex flex-col shadow-2xl'>
				<div className='flex justify-between items-center mb-4'>
					<h2 className='text-2xl font-bold'>Full Day Nutrition: {recipe.name}</h2>
				...
				<div className='overflow-y-auto flex-1 space-y-6'>
					<p className='text-sm text-gray-600 dark:text-gray-400'>
						This projects your nutrition if you only ate <strong>{recipe.name}</strong> to reach a <strong>{DAILY_GOAL} kcal</strong> goal. This would require approximately <strong>{servingsForGoal.toFixed(1)}</strong> servings.
					</p>
					...
					<div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
						<div className='flex flex-col items-center justify-center'>
							...
							<div className='w-full h-60'>
								<ResponsiveContainer>
									<BarChart data={macroData} layout='vertical' margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
										<CartesianGrid strokeDasharray='3 3' />
										<XAxis type='number' />
										<YAxis type='category' dataKey='name' width={60} />
										<Tooltip cursor={{ fill: 'rgba(240, 240, 240, 0.3)' }} />
										<Bar dataKey='value' fill='#8884d8' background={{ fill: '#eee' }} unit='g' />
									</BarChart>
								</ResponsiveContainer>
							</div>
						</div>
					...
					<div>
						...
						<div className='grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm'>
							<div className='bg-gray-50 dark:bg-gray-800 p-3 rounded-lg'>
								...
								<p className='font-bold text-lg'>{fullDayNutrition.calories.toFixed(0)} kcal</p>
							</div>
							<div className='bg-gray-50 dark:bg-gray-800 p-3 rounded-lg'>
								<p className='text-gray-600 dark:text-gray-400'>Protein</p>
								<p className='font-bold text-lg'>{fullDayNutrition.protein.toFixed(1)} g</p>
							</div>
							<div className='bg-gray-50 dark:bg-gray-800 p-3 rounded-lg'>
								<p className='text-gray-600 dark:text-gray-400'>Carbohydrates</p>
								<p className='font-bold text-lg'>{fullDayNutrition.carbs.toFixed(1)} g</p>
							</div>
							<div className='bg-gray-50 dark:bg-gray-800 p-3 rounded-lg'>
								<p className='text-gray-600 dark:text-gray-400'>Fat</p>
								<p className='font-bold text-lg'>{fullDayNutrition.fat.toFixed(1)} g</p>
							</div>
							<div className='bg-gray-50 dark:bg-gray-800 p-3 rounded-lg'>
								<p className='text-gray-600 dark:text-gray-400'>Fiber</p>
								<p className='font-bold text-lg'>{fullDayNutrition.fiber.toFixed(1)} g</p>
							</div>
							<div className='bg-gray-50 dark:bg-gray-800 p-3 rounded-lg'>
								<p className='text-gray-600 dark:text-gray-400'>Sugar</p>
								<p className='font-bold text-lg'>{fullDayNutrition.sugar.toFixed(1)} g</p>
							</div>
							<div className='bg-gray-50 dark:bg-gray-800 p-3 rounded-lg col-span-full'>
								...
								<p className='font-bold text-lg'>{fullDayNutrition.sodium.toFixed(0)} mg</p>
							</div>
						</div>
					</div>
				</div>
