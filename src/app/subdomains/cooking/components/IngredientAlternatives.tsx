...
export function IngredientAlternatives({
  recipeItemId,
  availableFoods,
  availableRecipes,
    ...
    const fetchAlternatives = async () => {
      ...
      try {
        const response = await fetch(
          `/api/cooking/recipes/alternatives?itemId=${recipeItemId}`
        );
          ...
          {alternatives.map((alt) => (
            <div key={alt.id} className="text-sm p-3 border rounded-md">
              <p className="font-medium">
                {alt.food?.name || alt.recipe?.name}
              </p>
              <p>
                {alt.amount} {alt.unit}
              </p>
              {alt.notes && (
                <p className="text-xs italic text-gray-500">"{alt.notes}"</p>
              )}
              <div className="flex items-center gap-2 mt-2">
                <Button
                  size="sm"
                  variant="ghost"
                  onClick={() => handleVote(alt.id, 'upvote')}
                >
                  👍 {alt.upvotes}
                </Button>
                <Button
                  size="sm"
                  variant="ghost"
                  onClick={() => handleVote(alt.id, 'downvote')}
                >
                  👎 {alt.downvotes}
                </Button>
              </div>
