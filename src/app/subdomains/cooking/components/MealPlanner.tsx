...
export function MealPlanner({
    ...
    const loadMealPlans = () => {
      if (isGuest) {
        ...
        if (storedPlans) {
          ...
          const datePlans = allPlans.filter(
            (plan) => plan.date === formatDateForAPI(selectedDate)
          );
        ...
        console.log('Loading meal plans from API not yet implemented');
      }
      setIsLoading(false);
    };
  ...
  const handleAddMeal = () => {
    ...
    if (isGuest) {
      ...
      const allPlans = storedPlans
        ? (JSON.parse(storedPlans) as MealPlan[])
        : [];
      ...
      console.log('Saving meal plan to API not yet implemented');
    }
  ...
  const handleRemoveMeal = (planId: string) => {
    if (isGuest) {
      ...
      console.log('Removing meal plan from API not yet implemented');
    }
  };

  const handleCopyToToday = async (plan: MealPlan) => {
    // This would add the meal plan as an actual entry in today's tracking
    console.log('Copy to today functionality not yet implemented', plan);
  };

  // Group meal plans by type
  const mealsByType = mealPlans.reduce(
    (acc, plan) => {
  ...
  const calculateTotals = () => {
    ...
    mealPlans.forEach((plan) => {
      ...
      if (plan.food) {
        ...
        calories +=
          plan.recipe.currentVersion.caloriesPerServing *
          plan.recipe.servings *
          multiplier;
        protein +=
          plan.recipe.currentVersion.proteinPerServing *
          plan.recipe.servings *
          multiplier;
        carbs +=
          plan.recipe.currentVersion.carbsPerServing *
          plan.recipe.servings *
          multiplier;
        fat +=
          plan.recipe.currentVersion.fatPerServing *
          plan.recipe.servings *
          multiplier;
      }
    });
                    ...
                    >
                      <div>
                        <p className="font-medium">
                          {plan.food?.name || plan.recipe?.name}
                        </p>
                        <p className="text-sm text-gray-600 dark:text-gray-400">
                          {plan.amount}g{plan.notes && ` • ${plan.notes}`}
                        </p>
                      </div>
                      <div className="flex gap-2">
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => handleCopyToToday(plan)}
                          title="Add to today's tracking"
                        >
                          📋
                        </Button>
                        <Button
              ...
              <Card className="p-4 space-y-4">
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    disabled={!selectedFoodId && !selectedRecipeId}
                  >
                    Add to Plan
                  </Button>
                  <Button
      ...
      {isGuest && (
        <div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
          <p className="text-sm text-yellow-800 dark:text-yellow-200">
            Your meal plans are saved locally. Sign in to sync them across
            devices.
          </p>
        </div>
      )}
