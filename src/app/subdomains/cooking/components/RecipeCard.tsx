...
export function RecipeCard({ recipe, onEdit, onCook, onDelete, onViewHistory, onViewFullDay, isOwner = false }: RecipeCardProps) {
	...
	const timeSummary = [
		{
			label: 'Prep',
			value: recipe.prepTime !== null && recipe.prepTime !== 0 ? formatDuration(recipe.prepTime) : null,
		},
		{
			label: 'Cook',
			value: recipe.cookTime !== null && recipe.cookTime !== 0 ? formatDuration(recipe.cookTime) : null,
		},
		{
			label: 'Total',
			value: getTotalTime(recipe.prepTime, recipe.cookTime) !== null && getTotalTime(recipe.prepTime, recipe.cookTime) !== 0 ? formatDuration(getTotalTime(recipe.prepTime, recipe.cookTime)) : null,
		},
		...
		<Card className='relative overflow-hidden hover-lift card-hover transition-all-300'>
			<CardHeader>
				<CardTitle className='flex justify-between items-start'>
					...
					>
						<ChevronDownIcon className={`transition-transform ${showDetails ? 'rotate-180' : ''}`} />
					</Button>
				</CardTitle>
			...
			<CardContent>
				<div className='flex justify-between text-sm text-gray-600 dark:text-gray-400'>
					<div className='flex items-center gap-1'>
						...
						<span>{timeSummary.find((t) => t.label === 'Total')?.value ?? 'N/A'}</span>
					</div>
					<div className='flex items-center gap-1'>
					...
					<div className='flex items-center gap-1'>
						...
						<span>{perServingNutrition?.calories.toFixed(0) ?? 'N/A'} cal/serving</span>
					</div>
				</div>
				{showDetails && (
					<div className='mt-4'>
						<div className='grid grid-cols-2 gap-4'>
							<div>
								...
								<ul className='list-disc list-inside text-sm'>
									{recipe.currentVersion?.items.slice(0, 5).map((item) => <li key={item.id}>{item.food?.name ?? item.recipe?.name}</li>)}
									{recipe.currentVersion && recipe.currentVersion.items.length > 5 && <li>...and {recipe.currentVersion.items.length - 5} more</li>}
								</ul>
							</div>
							<div>
								<h4 className='font-semibold mb-2'>Nutrition per Serving</h4>
								{perServingNutrition && <NutritionPieChart nutrition={perServingNutrition} />}
							</div>
						</div>
						{isOwner && (
			...
			{showDeleteConfirm && (
				<div className='absolute inset-0 bg-white/80 dark:bg-black/80 flex flex-col items-center justify-center p-4'>
					...
					<p className='text-sm text-center text-gray-600 dark:text-gray-400 mb-4'>This action cannot be undone.</p>
