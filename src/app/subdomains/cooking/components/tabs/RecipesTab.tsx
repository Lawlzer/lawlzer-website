...
import { NoRecipesEmptyState, NoSearchResultsEmptyState } from '../ui/EmptyState';
...
export function RecipesTab({ isCreatingRecipe, editingRecipe, recipes, filteredRecipes, recipeSearchTerm, loadingRecipes, availableFoods, isGuest, onCreateRecipe, onCancelCreate, onSaveRecipe, onUpdateRecipe, onCancelEdit, onEditRecipe, onDeleteRecipe, onCookRecipe, onViewHistory, onViewFullDay, onSearchChange, onSearchResults }: RecipesTabProps) {
	if (isCreatingRecipe) {
		return <RecipeCreator availableFoods={availableFoods} availableRecipes={recipes} onSave={onSaveRecipe} onCancel={onCancelCreate} />;
	}

	if (editingRecipe) {
		return <RecipeEditor recipe={editingRecipe} availableFoods={availableFoods} availableRecipes={recipes.filter((r) => r.id !== editingRecipe.id)} onSave={onUpdateRecipe} onCancel={onCancelEdit} />;
	}
	...
	return (
		<div className='space-y-4'>
			<div className='flex items-center justify-between'>
				...
				<button onClick={onCreateRecipe} className='px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors relative z-10'>
					Create Recipe
				</button>
			</div>
