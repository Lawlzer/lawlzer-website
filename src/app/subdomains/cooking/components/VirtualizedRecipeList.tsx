...
export const VirtualizedRecipeList: React.FC<VirtualizedRecipeListProps> = ({ recipes, isOwner = true, onEdit, onCook, onDelete, onViewHistory, onViewFullDay }) => {
	// Calculate grid dimensions based on viewport
	const [containerWidth, setContainerWidth] = React.useState(typeof window !== 'undefined' ? window.innerWidth - 48 : 1200); // Account for padding
	const [containerHeight, setContainerHeight] = React.useState(typeof window !== 'undefined' ? window.innerHeight - 300 : 800); // Account for header/footer
	...
	const Cell = useCallback(
		({ columnIndex, rowIndex, style }: { columnIndex: number; rowIndex: number; style: React.CSSProperties }) => {
			...
			return (
				...
				>
					<RecipeCard recipe={recipe} isOwner={isOwner} onEdit={onEdit ? () => onEdit(recipe) : undefined} onCook={onCook} onDelete={onDelete ? () => onDelete(recipe.id) : undefined} onViewHistory={onViewHistory ? () => onViewHistory(recipe) : undefined} onViewFullDay={() => onViewFullDay(recipe)} />
				</div>
			);
		},
		[columnCount, recipes, isOwner, onEdit, onCook, onDelete, onViewHistory, onViewFullDay]
	);
	...
	return (
		<Grid
			...
			className='scrollbar-thin scrollbar-thumb-muted-foreground'
		>
			{Cell}
		</Grid>
	);
