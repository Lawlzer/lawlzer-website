...
describe('ColorsPage Component', () => {
	...
	// Test the component rendering in isolation with a minimal approach
	it('should render loading state initially', async () => {
		// Import dynamically to avoid module-level issues
		const { default: ColorsPage } = await import('./page');

		// The component should render loading state
		const { container } = render(React.createElement(ColorsPage));

		// We expect the loading state to be shown
		const loadingElement = container.querySelector('.text-secondary-text');
		expect(loadingElement).toBeTruthy();
	});

	// Add more unit tests for individual functions if they were exported
	// For now, we're testing what we can access
});
