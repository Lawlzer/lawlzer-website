...
const getColorInputs = (page: Page): ColorInputs => ({
  ...
  primaryTextColor: page.locator(
    'label:has-text("✏️Text Color") input[type="color"]'
  ),
  primaryColor: page.locator(
    'label:has-text("🌟Primary Color") input[type="color"]'
  ),
  secondaryColor: page.locator(
    'label:has-text("🎭Secondary Background") input[type="color"]'
  ),
  secondaryTextColor: page.locator(
    'label:has-text("📝Secondary Text") input[type="color"]'
  ),
  borderColor: page.locator(
    'label:has-text("🖼️Border Color") input[type="color"]'
  ),
});
...
test.describe('Colors Page E2E Tests', () => {
  ...
  test('page loads healthily and has correct metadata', async ({ page }) => {
    await testPageBasics(page, colorsPageUrl, expectedMetadata, {
      noScrollbarCheck: false,
    });
    await expect(
      page.locator('h1:has-text("Color Theme Studio")')
    ).toBeVisible();
  });
  ...
  test('should display default colors on initial load', async ({ page }) => {
    ...
    await expect(inputs.primaryColor).toHaveValue(
      LIGHT_MODE_COLORS.PRIMARY_COLOR
    );
    await expect(inputs.secondaryColor).toHaveValue(
      LIGHT_MODE_COLORS.SECONDARY_COLOR
    );
    await expect(inputs.primaryTextColor).toHaveValue(
      LIGHT_MODE_COLORS.PRIMARY_TEXT_COLOR
    );
    await expect(inputs.secondaryTextColor).toHaveValue(
      LIGHT_MODE_COLORS.SECONDARY_TEXT_COLOR
    );
    await expect(inputs.borderColor).toHaveValue(
      LIGHT_MODE_COLORS.BORDER_COLOR
    );

    // Check if body background matches
    await checkElementStyle(
      page,
      'body',
      'background-color',
      LIGHT_MODE_COLORS.PAGE_BG
    );
  ...
  test('should update color input value and page style on change', async ({
    page,
  }) => {
    const inputs = getColorInputs(page);
  ...
  test('should apply a predefined palette correctly', async ({ page }) => {
    ...
    await expect(inputs.secondaryColor).toHaveValue(
      lightPalette.SECONDARY_COLOR
    );
    await expect(inputs.primaryTextColor).toHaveValue(
      lightPalette.PRIMARY_TEXT_COLOR
    );
    await expect(inputs.secondaryTextColor).toHaveValue(
      lightPalette.SECONDARY_TEXT_COLOR
    );
    ...
    await checkElementStyle(
      ...
      'body',
      'background-color',
      lightPalette.PAGE_BG
    );
  });

  test('should save colors to cookies and show success message', async ({
    page: _page,
    context: _context,
  }) => {
    // Skip this test as the Save button no longer exists in the new UI
    test.skip();
  });

  test('should load saved colors from cookies on reload', async ({
    page,
    context,
  }) => {
    const inputs = getColorInputs(page);
    ...
    await context.addCookies([
      {
        name: COOKIE_KEYS.PAGE_BG,
        value: testColors[COOKIE_KEYS.PAGE_BG],
        url: colorsPageUrl,
      },
      {
        name: COOKIE_KEYS.PRIMARY_COLOR,
        value: testColors[COOKIE_KEYS.PRIMARY_COLOR],
        url: colorsPageUrl,
      },
      {
        name: COOKIE_KEYS.SECONDARY_COLOR,
        value: testColors[COOKIE_KEYS.SECONDARY_COLOR],
        url: colorsPageUrl,
      },
      {
        name: COOKIE_KEYS.PRIMARY_TEXT_COLOR,
        value: testColors[COOKIE_KEYS.PRIMARY_TEXT_COLOR],
        url: colorsPageUrl,
      },
      {
      ...
      {
        name: COOKIE_KEYS.BORDER_COLOR,
        value: testColors[COOKIE_KEYS.BORDER_COLOR],
        url: colorsPageUrl,
      },
    ]);
    ...
    await expect(inputs.primaryColor).toHaveValue(
      testColors[COOKIE_KEYS.PRIMARY_COLOR]
    );
    await expect(inputs.secondaryColor).toHaveValue(
      testColors[COOKIE_KEYS.SECONDARY_COLOR]
    );
    await expect(inputs.primaryTextColor).toHaveValue(
      testColors[COOKIE_KEYS.PRIMARY_TEXT_COLOR]
    );
    await expect(inputs.secondaryTextColor).toHaveValue(
      testColors[COOKIE_KEYS.SECONDARY_TEXT_COLOR]
    );
    await expect(inputs.borderColor).toHaveValue(
      testColors[COOKIE_KEYS.BORDER_COLOR]
    );
  });
  ...
  test('Export button shows success message', async ({ page }) => {
    ...
    await expect(page.getByText('Colors copied to clipboard!')).toBeVisible({
      timeout: 5000,
    });
  });

  test('Import button shows success message (requires clipboard mock/permission)', async ({
    page,
  }) => {
    ...
    await expect(page.getByText('Colors imported successfully!')).toBeVisible({
      timeout: 10000,
    });
  ...
  test('Import button shows error message with invalid data', async ({
    page,
  }) => {
    ...
    await expect(page.getByText(/Failed to import colors/i)).toBeVisible({
      timeout: 5000,
    });
  });

  test('should save color scheme to cookies and show success message', async ({
    page: _page,
    context: _context,
  }) => {
    // Skip this test as the Color Scheme dropdown no longer exists in the new UI
    test.skip();
  });
});
