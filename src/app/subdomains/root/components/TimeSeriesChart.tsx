...
function computeYScaleParams(formattedData: NonNullable<FormattedChartData>, hiddenDatasets: Set<string>, activeChartTab: string): { minY: number; maxY: number; range: number; numYTicks: number } {
	let minY = Infinity;
	let maxY = -Infinity;
	...
	if (range === 0) {
		yPadding = Math.abs(minY) * 0.1 === 0 ? 1 : Math.abs(minY) * 0.1;
	} else if (range < 0.1) {
		yPadding = range * 0.5;
	} else if (range < 1) {
		yPadding = range * 0.3;
	...
	return { minY, maxY, range, numYTicks };
}

function formatTickValue(num: number, range: number): string {
	if (range < 0.01) return num.toFixed(4);
	if (range < 0.1) return num.toFixed(3);
	if (range < 1) return num.toFixed(2);
	if (range < 10) return num.toFixed(1);
	if (num >= 1000000) return `${(num / 1000000).toFixed(1)}M`;
	if (num >= 1000) return `${(num / 1000).toFixed(0)}K`;
	return Math.round(num).toString();
}

function ChartLegend({ formattedData, hiddenDatasets, handleLegendClick, isMobile }: { formattedData: NonNullable<FormattedChartData>; hiddenDatasets: Set<string>; handleLegendClick: (label: string) => void; isMobile: boolean }): React.JSX.Element {
...
function ChartTooltip({ tooltipData, tooltipLeft, tooltipTop, range, formattedData, TooltipInPortal }: { tooltipData: CombinedTooltipData; tooltipLeft: number | undefined; tooltipTop: number | undefined; range: number; formattedData: NonNullable<FormattedChartData>; TooltipInPortal: React.FC<any> }): React.JSX.Element {
...
export const TimeSeriesChart: React.FC<TimeSeriesChartProps> = ({ formattedData, activeChartTab, hiddenDatasets, handleLegendClick, isMobile, tooltipData, tooltipLeft, tooltipTop, tooltipOpen, hideTooltip, handleTooltip, containerRef, TooltipInPortal }) => {
	...
	return <ParentSize>{({ width, height }) => <TimeSeriesChartInner width={width} height={height} formattedData={formattedData} activeChartTab={activeChartTab} hiddenDatasets={hiddenDatasets} handleLegendClick={handleLegendClick} isMobile={isMobile} tooltipData={tooltipData} tooltipLeft={tooltipLeft} tooltipTop={tooltipTop} tooltipOpen={tooltipOpen} hideTooltip={hideTooltip} handleTooltip={handleTooltip} containerRef={containerRef} TooltipInPortal={TooltipInPortal} />}</ParentSize>;
};

function TimeSeriesChartInner({ width, height, formattedData, activeChartTab, hiddenDatasets, handleLegendClick, isMobile, tooltipData, tooltipLeft, tooltipTop, tooltipOpen, hideTooltip, handleTooltip, containerRef, TooltipInPortal }: { width: number; height: number; formattedData: NonNullable<FormattedChartData>; activeChartTab: string; hiddenDatasets: Set<string>; handleLegendClick: (label: string) => void; isMobile: boolean; tooltipData: CombinedTooltipData | undefined; tooltipLeft: number | undefined; tooltipTop: number | undefined; tooltipOpen: boolean; hideTooltip: () => void; handleTooltip: (event: React.MouseEvent<SVGRectElement> | React.TouchEvent<SVGRectElement>, chartWidth: number, chartHeight: number, xScale: any, yScale: any) => void; containerRef: (element: SVGSVGElement | null) => void; TooltipInPortal: React.FC<any> }): React.JSX.Element {
	if (width === 0 || height === 0) {
	...
	const { minY, maxY, range, numYTicks } = computeYScaleParams(formattedData, hiddenDatasets, activeChartTab);
	...
	return (
		<>
			<ChartLegend formattedData={formattedData} hiddenDatasets={hiddenDatasets} handleLegendClick={handleLegendClick} isMobile={isMobile} />
			<svg ref={containerRef} height={height} width={width}>
				<Group left={chartMargin.left} top={chartMargin.top}>
					...
					<AxisBottom label='Day of Year' numTicks={innerWidth > 520 ? 12 : 6} scale={xScale} tickFormat={formatDayOfYearForAxis} stroke='currentColor' tickStroke='currentColor' top={innerHeight} labelProps={{ fontSize: 14, fill: 'currentColor', textAnchor: 'middle', dy: '2.5em', className: 'text-foreground' }} tickLabelProps={{ fontSize: 11, textAnchor: 'middle', dy: '0.25em', angle: innerWidth < 400 ? 45 : 0, fill: 'currentColor', className: 'text-muted-foreground' }} />
					<AxisLeft label={getYAxisLabel(activeChartTab)} numTicks={numYTicks} scale={yScale} stroke='currentColor' tickStroke='currentColor' labelProps={{ fontSize: 14, fill: 'currentColor', textAnchor: 'middle', dy: '-3em', dx: '-3em', angle: -90, className: 'text-foreground' }} tickLabelProps={{ fontSize: 11, textAnchor: 'end', dx: '-0.25em', dy: '0.25em', fill: 'currentColor', className: 'text-muted-foreground' }} tickFormat={(value) => formatTickValue(typeof value === 'number' ? value : value.valueOf(), range)} />
			...
			{tooltipOpen && tooltipData ? <ChartTooltip tooltipData={tooltipData} tooltipLeft={tooltipLeft} tooltipTop={tooltipTop} range={range} formattedData={formattedData} TooltipInPortal={TooltipInPortal} /> : null}
		</>
	);
}
