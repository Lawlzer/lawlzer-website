...
// SVG icon paths used in the panel
const chartIconPath = 'M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z';
const barChartIconPath = 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z';
...
const filterIconPath = 'M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4';
const dbIconPath = 'M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4';

function MobileViewFiltersButton({ ButtonComponent, handleToggleMobileView, animationsEnabled }: { ButtonComponent: any; handleToggleMobileView: () => void; animationsEnabled: boolean }): React.JSX.Element {
	return (
		<ButtonComponent type='button' className='mt-6 inline-flex items-center gap-2 rounded-xl bg-primary/10 px-4 py-2 text-sm font-medium text-primary transition-all hover:bg-primary/20' onClick={handleToggleMobileView} {...(animationsEnabled ? { whileHover: { scale: 1.05 }, whileTap: { scale: 0.95 } } : {})}>
			<svg className='h-4 w-4' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
				<path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d={filterIconPath} />
			</svg>
			View Filters
		</ButtonComponent>
	);
...
function ChartPanelHeader({ activeChartTab, ItemDiv, animationsEnabled }: { activeChartTab: string | null; ItemDiv: any; animationsEnabled: boolean }): React.JSX.Element {
	return (
				...
				<h3 className='flex items-center gap-2 text-sm font-semibold text-foreground'>
					<ItemDiv className='flex h-6 w-6 items-center justify-center rounded-md bg-primary/10' {...(animationsEnabled ? { whileHover: { scale: 1.1, rotate: -10 }, transition: { type: 'spring', stiffness: 200 } } : {})}>
						<svg className='h-4 w-4 text-primary' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
							<path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2.5} d={chartIconPath} />
						</svg>
					</ItemDiv>
					Data Visualization
				</h3>
				{activeChartTab !== null && activeChartTab !== '' && (
					<ItemDiv className='flex items-center gap-1.5 rounded-full bg-primary/10 px-2.5 py-1 text-xs font-medium text-primary' {...(animationsEnabled ? { initial: { scale: 0, opacity: 0 }, animate: { scale: 1, opacity: 1 }, transition: { type: 'spring', stiffness: 300 } } : {})}>
						<svg className='h-3 w-3' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
							<path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d={barChartIconPath} />
						</svg>
...
function ChartLimitExceededMessage({ ItemDiv, ButtonComponent, chartDocumentCount, isMobile, handleToggleMobileView, animationsEnabled }: { ItemDiv: any; ButtonComponent: any; chartDocumentCount: number; isMobile: boolean; handleToggleMobileView: () => void; animationsEnabled: boolean }): React.JSX.Element {
	return (
		<>
			...
			{isMobile && <MobileViewFiltersButton ButtonComponent={ButtonComponent} handleToggleMobileView={handleToggleMobileView} animationsEnabled={animationsEnabled} />}
		</>
	);
}

function ChartGenericMessage({ ItemDiv, ButtonComponent, chartMessage, isMobile, canShowChartBasedOnFilters, handleToggleMobileView, animationsEnabled }: { ItemDiv: any; ButtonComponent: any; chartMessage: string; isMobile: boolean; canShowChartBasedOnFilters: boolean; handleToggleMobileView: () => void; animationsEnabled: boolean }): React.JSX.Element {
	return (
		<>
			<ItemDiv className='mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-muted/50 to-muted/30' {...(animationsEnabled ? { animate: { rotate: [0, 360] }, transition: { duration: 30, repeat: Infinity, ease: 'linear' } } : {})}>
				<svg className='h-10 w-10 text-muted-foreground' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
					<path strokeLinecap='round' strokeLinejoin='round' strokeWidth={1.5} d={barChartIconPath} />
				</svg>
			</ItemDiv>
			<p className='text-base font-medium text-muted-foreground'>{chartMessage}</p>
			{isMobile && !canShowChartBasedOnFilters && <MobileViewFiltersButton ButtonComponent={ButtonComponent} handleToggleMobileView={handleToggleMobileView} animationsEnabled={animationsEnabled} />}
		</>
	);
}

function ChartPanelFooter({ ItemDiv, totalDocuments, chartLimitExceeded, canShowChartBasedOnFilters, showCharts, getFormattedChartData, hiddenDatasets, animationsEnabled }: { ItemDiv: any; totalDocuments: number; chartLimitExceeded: boolean; canShowChartBasedOnFilters: boolean; showCharts: boolean; getFormattedChartData: FormattedChartData; hiddenDatasets: Set<string>; animationsEnabled: boolean }): React.JSX.Element {
	return (
			...
			<div className='flex items-center gap-3 text-[10px] text-muted-foreground'>
				<span className='flex items-center gap-1.5'>
					<svg className='h-3.5 w-3.5 text-primary' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
						<path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d={dbIconPath} />
					</svg>
...
function ChartPanelEmptyState({ ItemDiv, ButtonComponent, totalDocuments, isMobile, handleToggleMobileView, animationsEnabled }: { ItemDiv: any; ButtonComponent: any; totalDocuments: number; isMobile: boolean; handleToggleMobileView: () => void; animationsEnabled: boolean }): React.JSX.Element {
	return (
		<ItemDiv className='flex flex-grow items-center justify-center p-4' {...(animationsEnabled ? { initial: { opacity: 0 }, animate: { opacity: 1 } } : {})}>
			<div className='text-center'>
				<ItemDiv className='mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-muted/50 to-muted/30' {...(animationsEnabled ? { animate: { rotate: [0, -10, 10, -10, 0] }, transition: { duration: 4, repeat: Infinity, repeatDelay: 2 } } : {})}>
					<svg className='h-10 w-10 text-muted-foreground' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
						<path strokeLinecap='round' strokeLinejoin='round' strokeWidth={1.5} d={filterIconPath} />
					</svg>
				</ItemDiv>
...
export const ChartPanel: React.FC<ChartPanelProps> = ({ isLoading, error: _error, showCharts, chartLimitExceeded, canShowChartBasedOnFilters, chartableFields, activeChartTab, handleChartTabChange, chartMessage, getFormattedChartData, isMobile, mobileViewMode, handleToggleMobileView, chartDocumentCount, totalDocuments, changingChartTabVisual, hiddenDatasets, handleLegendClick, tooltipData, tooltipLeft, tooltipTop, tooltipOpen, hideTooltip, handleTooltip, containerRef, TooltipInPortal, animationsEnabled }) => {
	...
	return (
		<ContainerDiv className={`relative flex flex-col h-full overflow-hidden rounded-2xl border border-border/50 bg-gradient-to-br from-card/95 to-card/98 shadow-lg`} {...(animationsEnabled ? { initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.3, delay: 0.1 } } : {})}>
			<ChartPanelHeader activeChartTab={activeChartTab} ItemDiv={ItemDiv} animationsEnabled={animationsEnabled} />
			{shouldShowPanelContent ? (
				<div className='flex flex-grow flex-col overflow-hidden'>
							...
							<div className='flex space-x-1.5 overflow-x-auto p-2'>
								{chartableFields.map((key, index) => (
									<ButtonComponent
										...
										{...(animationsEnabled ? { initial: { opacity: 0, x: -20 }, animate: { opacity: 1, x: 0 }, transition: { delay: index * 0.05, type: 'spring', stiffness: 200 }, whileHover: { scale: 1.05 }, whileTap: { scale: 0.95 } } : {})}
									>
										{formatFieldName(key)}
					...
					{isLoading ? (
						...
						<ItemDiv className={`flex flex-grow items-center justify-center p-4 ${isMobile ? 'text-sm' : ''}`} {...(animationsEnabled ? { initial: { opacity: 0 }, animate: { opacity: 1 }, transition: { delay: 0.2 } } : {})}>
							<div className='text-center'>{chartLimitExceeded ? <ChartLimitExceededMessage ItemDiv={ItemDiv} ButtonComponent={ButtonComponent} chartDocumentCount={chartDocumentCount} isMobile={isMobile} handleToggleMobileView={handleToggleMobileView} animationsEnabled={animationsEnabled} /> : <ChartGenericMessage ItemDiv={ItemDiv} ButtonComponent={ButtonComponent} chartMessage={chartMessage} isMobile={isMobile} canShowChartBasedOnFilters={canShowChartBasedOnFilters} handleToggleMobileView={handleToggleMobileView} animationsEnabled={animationsEnabled} />}</div>
						</ItemDiv>
						...
						<ItemDiv className='relative flex-grow bg-gradient-to-br from-background/20 to-background/5' {...(animationsEnabled ? { initial: { opacity: 0 }, animate: { opacity: 1 }, transition: { delay: 0.3 } } : {})}>
							<TimeSeriesChart TooltipInPortal={TooltipInPortal} activeChartTab={activeChartTab} containerRef={containerRef} formattedData={getFormattedChartData} handleLegendClick={handleLegendClick} handleTooltip={handleTooltip} hiddenDatasets={hiddenDatasets} hideTooltip={hideTooltip} tooltipLeft={tooltipLeft} tooltipOpen={tooltipOpen} tooltipTop={tooltipTop} isMobile={isMobile} tooltipData={tooltipData} />
				...
				<ChartPanelEmptyState ItemDiv={ItemDiv} ButtonComponent={ButtonComponent} totalDocuments={totalDocuments} isMobile={isMobile} handleToggleMobileView={handleToggleMobileView} animationsEnabled={animationsEnabled} />
			)}
			<ChartPanelFooter ItemDiv={ItemDiv} totalDocuments={totalDocuments} chartLimitExceeded={chartLimitExceeded} canShowChartBasedOnFilters={canShowChartBasedOnFilters} showCharts={showCharts} getFormattedChartData={getFormattedChartData} hiddenDatasets={hiddenDatasets} animationsEnabled={animationsEnabled} />
		</ContainerDiv>
	);
};
