...
async function setupMockRoutes(
  ...
  const {
    filtersResponse = mockSuccessFilters,
    chartDataResponse = mockSuccessChartData,
    filtersStatus = 200,
    chartDataStatus = 200,
  } = options;
...
test.describe('DataPlatformPreview E2E Tests', () => {
  test('should hide charts when document count exceeds limit', async ({
    page,
  }): Promise<void> => {
    ...
    await expect(
      page.getByRole('button', { name: 'Europe (25)' })
    ).toBeVisible();

    // Check for the warning message
    await expect(
      page.getByText(
        /Chart generation disabled.*Dataset size.*exceeds the limit/i
      )
    ).toBeVisible();
  ...
  test('should display "No data" message when API returns empty', async ({
    page,
  }): Promise<void> => {
    await setupMockRoutes(page, {
      filtersResponse: mockEmptyFilters,
      chartDataResponse: mockEmptyChartData,
    });
    ...
    await expect(
      page
        .locator('.lg\\:col-span-1')
        .getByText(/No documents match the current filters./i)
    ).toBeVisible();
    await expect(
      page.getByRole('button', { name: /Europe/i })
    ).not.toBeVisible(); // Data shouldn't render
  });
});
