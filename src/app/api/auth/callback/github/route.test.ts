...
describe('GitHub OAuth Callback', () => {
  ...
  it('should handle successful GitHub OAuth callback', async () => {
    ...
    (
      handleAndGenerateSessionToken as ReturnType<typeof vi.fn>
    ).mockResolvedValue(
      new Response(null, {
        status: 302,
        headers: {
    ...
    expect(handleAndGenerateSessionToken).toHaveBeenCalledWith(
      mockUser.id,
      expect.any(Object)
    );
  });
  ...
  it('should handle database upsert failure', async () => {
    ...
    expect(location).toContain(
      '/error/auth?error=Database%20connection%20failed'
    );
  ...
  it('should handle user without verified email', async () => {
    ...
    const location = response.headers.get('location');
    expect(location).toContain(
      '/error/auth?error=Could%20not%20retrieve%20a%20verified%20primary%20email%20from%20GitHub.'
    );
