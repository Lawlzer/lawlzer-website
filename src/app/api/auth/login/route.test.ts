...
describe('/api/auth/login', () => {
	...
	it('should redirect to Discord OAuth with correct parameters', async () => {
		...
		await testApiHandler({
			appHandler: { GET } as any,
			url: '/api/auth/login?provider=discord',
			test: async ({ fetch }) => {
				const response = await fetch({
					method: 'GET',
					redirect: 'manual',
	...
	it('should throw error for unsupported provider', async () => {
		...
		await testApiHandler({
			appHandler: { GET } as any,
			url: '/api/auth/login?provider=unsupported',
			test: async ({ fetch }) => {
				// This will throw an unhandled error internally
				const response = await fetch({
					method: 'GET',
				});

				// The error will bubble up as a 500
				expect(response.status).toBe(500);
			},
		});
