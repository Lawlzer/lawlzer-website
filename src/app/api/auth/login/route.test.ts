...
describe('/api/auth/login', () => {
  ...
  const _mockGetCookieDomain = authLib.getCookieDomain as ReturnType<
    typeof vi.fn
  >;
  ...
  it('should set auth cookies correctly', async () => {
    ...
    const authRedirectCookie = cookies.find((c) =>
      c.startsWith('auth_redirect=')
    );
    expect(authRedirectCookie).toBeTruthy();
    expect(authRedirectCookie).toContain(
      'auth_redirect=http%3A%2F%2Flocalhost%3A3000%2Fdashboard'
    );
