...
describe('/api/auth/session', () => {
  ...
  it('should return session data for valid session token', async () => {
    ...
    (getSessionDataByToken as ReturnType<typeof vi.fn>).mockResolvedValue(
      mockSessionData
    );

    const { GET } = await import('./route');

    const response = await testRoute({ GET }, '/api/auth/session', {
      headers: {
  ...
  it('should handle database errors gracefully', async () => {
    ...
    const { getSessionDataByToken } = await import('~/server/db/session');
    (getSessionDataByToken as ReturnType<typeof vi.fn>).mockRejectedValue(
