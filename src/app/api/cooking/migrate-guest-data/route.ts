...
import type { GuestData } from '~/app/subdomains/cooking/services/guestStorage';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const hasValidStructure =
      (guestData.foods === undefined || Array.isArray(guestData.foods)) &&
      (guestData.recipes === undefined || Array.isArray(guestData.recipes)) &&
      (guestData.days === undefined || Array.isArray(guestData.days));
      ...
      if (Array.isArray(guestData.foods)) {
        for (const food of guestData.foods) {
          try {
            ...
            // Skip invalid foods
            if (!food.name || !foodGuestId) continue;
      ...
      if (Array.isArray(guestData.recipes)) {
        for (const recipe of guestData.recipes) {
          try {
            ...
            if (!recipe.name || !recipeGuestId) continue;
            ...
            const itemCreates = Array.isArray(items)
              ? items.map((item: any) => {
                  ...
                  const foodId = item.foodId
                    ? guestFoodIdToDbId.get(item.foodId)
                    : null;
                  return {
            ...
            if (Array.isArray(day.entries)) {
              for (const entry of day.entries) {
                const foodId = entry.foodId
                  ? guestFoodIdToDbId.get(entry.foodId)
                  : null;
                const recipeVersionId = entry.recipeId
                  ? guestRecipeIdToDbVersionId.get(entry.recipeId)
                  : null;

                if (foodId || recipeVersionId) {
