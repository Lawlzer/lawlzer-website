...
describe('/api/cooking/fridge', () => {
  ...
  describe('GET', () => {
    ...
    it('should handle database errors', async () => {
      (getSession as any).mockResolvedValue(mockSession);
      (db.fridgeItem.findMany as any).mockRejectedValue(
  ...
  describe('POST', () => {
    ...
    it('should handle database errors on create', async () => {
      (getSession as any).mockResolvedValue(mockSession);
      (db.fridgeItem.create as any).mockRejectedValue(
        new Error('Database error')
      );
  ...
  describe('DELETE', () => {
    it('should return 401 if user is not authenticated', async () => {
      ...
      const request = new Request(
        'http://localhost:3000/api/cooking/fridge?id=item-1',
        {
          method: 'DELETE',
        }
      );
    ...
    it('should handle database errors on delete', async () => {
      (getSession as any).mockResolvedValue(mockSession);
      (db.fridgeItem.delete as any).mockRejectedValue(
        new Error('Database error')
      );
