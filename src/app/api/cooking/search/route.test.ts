...
describe('/api/cooking/search', () => {
  ...
  describe('GET', () => {
    it('should return all public recipes when no filters are applied', async () => {
      ...
      (db.recipe.findMany as any).mockResolvedValue(
        mockRecipes.filter((r) => r.visibility === 'public')
      );

      const request = new NextRequest(
        'http://localhost:3000/api/cooking/search'
      );
    ...
    it('should filter by query string', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/cooking/search?query=pasta'
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should filter by ingredients', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/cooking/search?ingredients=tomato,onion'
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should filter by max prep time', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/cooking/search?maxPrepTime=15'
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should sort by likes when specified', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/cooking/search?sortBy=likes'
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should handle combined filters', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/cooking/search?query=pasta&ingredients=pasta&maxPrepTime=20&sortBy=likes'
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should handle database errors', async () => {
      (getSession as any).mockResolvedValue(null);
      (db.recipe.findMany as any).mockRejectedValue(
    ...
    it('should exclude component recipes', async () => {
      (getSession as any).mockResolvedValue(null);
      (db.recipe.findMany as any).mockResolvedValue(
        mockRecipes.filter((r) => !r.isComponent)
      );
