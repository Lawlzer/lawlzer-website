...
import {
  cleanupTestUserAndSession,
  seedTestUserAndSession,
} from '../../testUtils/playwright/utils';
...
test.describe('Navigation Flow', () => {
  ...
  test('should navigate between pages while maintaining authentication', async ({
    page,
    context,
  }) => {
    ...
    await page.goto('/');
    await expect(
      page.getByRole('button', { name: /test e2e user/i })
    ).toBeVisible();
    ...
    await expect(page.locator('svg.max-h-full').first()).toBeVisible({
      timeout: 20000,
    });
    await expect(
      page.getByRole('button', { name: /test e2e user/i })
  ...
  test('should preserve authentication state across browser refresh', async ({
    page,
    context,
  }) => {
    // Seed test user and session
  ...
  test('should show correct user menu options when authenticated', async ({
    page,
    context,
  }) => {
    // Seed test user and session
